...
export const Orders = async ({ userId }: { userId: string }) => {
  ...
  return (
    <div className="flex w-full justify-center items-center flex-col">
      <h1 className="text-main text-3xl font-semibold">Your Orders</h1>
      {purchases.length === 0 ? (
        ...
        purchases.map((purchase) => (
          <div key={purchase.id} className="pt-4 w-full">
            ...
            <div className="flex justify-between items-center">
              <h2 className="text-xl font-semibold mb-4">Order ID: {purchase.id}</h2>
              <p className="text-xl font-semibold mb-4" >Created At: {purchase.createdAt.toLocaleDateString()}</p>
              <p className="text-xl font-semibold mb-4" >Total Price - ${purchase.totalPrice}</p>
            </div>
            <Table>
              <TableHeader>
                <TableRow>
