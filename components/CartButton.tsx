...
export const CartButton = () => {
  ...
  return (
    <Link href={"/cart"} className="flex items-center justify-center gap-1 text-sm font-medium group whitespace-nowrap ring-offset-background transition-colors 
     ...
     disabled:opacity-50 text-[#1d3966] bg-[#d9eaff] w-max rounded-full hover:bg-[#1d3966] hover:text-white h-10 px-6 py-2">
      ...
      {items.length === 0 ? <p>{" "}</p> : <p>{items.length}</p>}
    </Link>
  )
}
